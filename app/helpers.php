...
use Illuminate\Support\Facades\DB;
  ...
  function verifyPhoneNumber($phone, $otp)
  {
      ...
      try{
         ...
         $verification = MobileVerification::where([
            'phone' => $phone,
            'otp' => $otp
         ])->first();
  ...
  function getProfile($id){
      ...
      $user['mason_category']['name']=getMasonCategoryName($user['points']);
      ksort($user);
      return $user;

  }

   function getMasonCategoryName($point)
  {
   $mason_category=MasonCategory::where('from_point','<=', $point)
      ->where('to_point','>=', $point)->first();
      return $mason_category->name ?? "";
  }
