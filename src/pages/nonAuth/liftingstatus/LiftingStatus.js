...
import styles from './LiftingStatusStyle'
...
const LiftingStatus = props => {
        ...
        getApiWithHeader(url)
            .then(response => {
                if (response.data) {
                    ...
                    setMasonList((masonList) => products)
                }
            })
            .catch(err => {
                setLoading(false)
    ...
    const callRejectLiftingApi = item => {
        setLoading(true)
        let newData = item.dataItem
        var FormData = require('form-data')
        var formData = new FormData()
        formData.append('lifting_id', newData[0].value)
        formData.append('preferred_app_lang', selectedLanguage())
        postApiWithHeader(Constants.reject_lifting, formData)
            .then(response => {
                setLoading(false)
                if (response.data.status) {
                    Toast.show({
    ...
    const goToEditLifting = item => {
        props.navigation.navigate('EditLiftingStatus', { data: item })
    }
            ...
            <>
                {item.key !== 'lifting_id' ? <View style={index % 2 == 1 ? { flexDirection: 'row', flex: 2, minHeight: 40, padding: 5, justifyContent: 'center', alignItems: 'center' } : { flexDirection: 'row', flex: 2, minHeight: 40, padding: 5, backgroundColor: '#FFF0F1', borderRadius: 5, justifyContent: 'center', alignItems: 'center' }}>
                    <View style={{ flex: 1, height: '100%', flexDirection: 'column', justifyContent: 'center' }}>
                        <Text style={{ color: 'gray', fontSize: 15 }}>{convertForShowData(item.key)}</Text>
                    </View>
                ...
                <View style={{ width: '100%', height: 70 }}>
                    <View style={{ width: '100%', alignItems: 'center', justifyContent: 'center', height: '100%' }}>
                        <Text style={{
                            ...
                            fontWeight: '600',
                            marginBottom: 20
                        }}>{convertForShowData(textValue.LIFTING_APPROVAL)}</Text>
                    </View>
                ...
                <View style={{ width: '100%', flex: 1, paddingHorizontal: 30 }}>
                    <View style={{ width: '100%', height: '100%', backgroundColor: '#FFF', borderTopLeftRadius: 20, borderTopRightRadius: 20, paddingVertical: 15, paddingHorizontal: 10 }}>
                        <View style={{ flexDirection: 'row', paddingVertical: 10, justifyContent: 'space-between', alignItems: 'center' }}>
                            ...
                            <TouchableOpacity
                                onPress={() => {
                                    DataStore.value = 'Approved'
                                    setLiftingState('Approved')
                                    setLabel('')
                                    setLoading(true)
                                    setTimeout(() => {
                                        getLiftingApproved(1, [])
                                    }, 1000)
                                }}
                            ...
                            <TouchableOpacity
                                onPress={() => {
                                    DataStore.value = 'Reject'
                                    setLiftingState('Reject')
                                    setLabel('')
                                    setLoading(true)
                                    setTimeout(() => {
                                        getRejectedLifting(1, [])
                                    }, 1000)
                                }}
                        ...
                        <>
                            <TouchableOpacity onPress={() => {
                                ...
                                <Text style={{ flex: 1, color: '#000' }}>{label == '' ? textValue.Select_Mason : label}</Text>
