...
import { ScrollView, Text, TextInput, View, Image, TouchableOpacity, Platform, Modal, TouchableWithoutFeedback, FlatList, KeyboardAvoidingView, Keyboard, Dimensions } from 'react-native'
        ...
        getApiWithHeader(constants.app_registration_link_visible + '?preferred_app_lang=' + selectedLanguage())
            .then(response => {
                if (response.data.status) {
                    ...
                    if (response?.data?.data[0]?.lifting_by_otp_button == '1') {
                        setOtpViewbtn(true)
                    }
                    setMax(response?.data?.data[0]?.maximum_lifting_limit)
                    setMin(response?.data?.data[0]?.minimum_lifting_limit)
                    setLiftingLimit(response?.data?.data[0]?.normal_lifting_limit)
                }
            })
            .catch(err => { })
    }
        ...
        if (valueDealerRssd == null) {
            ...
            if (parseInt(convertForUploadData(max)) > 0) {
                if (parseInt(convertForUploadData(qty)) >= parseInt(convertForUploadData(min)) && parseInt(convertForUploadData(qty)) <= parseInt(convertForUploadData(max))) {
                    if (parseInt(convertForUploadData(qty)) <= parseInt(convertForUploadData(liftingLimit))) {
            ...
            <View style={{ width: '100%', height: '100%', position: 'absolute', flexDirection: 'column' }}>
                ...
                <View style={{ width: '100%', height: 70 }}>
                    <View style={{ width: '100%', alignItems: 'center', justifyContent: 'center', height: '100%' }}>
                        <Text style={{ fontSize: 20, color: '#fff', fontWeight: '600', marginBottom: 20 }}>{convertForShowData(textValue.ADD_LIFTING)}</Text>
                    </View>
                ...
                <TouchableWithoutFeedback>
                    <View style={{ height: '100%', width: '100%', backgroundColor: '#00000095', justifyContent: 'center', alignItems: 'center', position: 'relative' }}>
                        <View style={{ width: '90%', backgroundColor: '#fff', borderRadius: 10, alignItems: 'center', padding: 10 }}>
                            ...
                            {otpView === true ? <View style={{ width: '70%', height: 50, marginTop: '2%', marginLeft: 10, }}>
                                <OtpInputs
                                    clearTextOnFocus
                            ...
                            {otpView === true ? <TouchableOpacity onPress={() => {
                                verify_otp()
                            ...
                            {otpView === false && otpViewbtn === true ? <TouchableOpacity onPress={() => {
                                setOtpView(true)
                                send_otp()
                            }} style={styles._lowerView._btn} activeOpacity={0.8}>
                            ...
                            {starSathibtn == true && otpView == false ? <TouchableOpacity onPress={() => { callLifting() }} style={styles._lowerView._btn} activeOpacity={0.8}>
                    ...
                    <View style={{ height: '100%', width: '100%', backgroundColor: '#00000095', justifyContent: 'center', alignItems: 'center', position: 'relative' }}>
                        <View style={{ width: '80%', padding: 20, backgroundColor: '#FFF', borderRadius: 10 }}>
                            ...
                            <View style={{ width: '100%', flexDirection: 'row' }}>
                                ...
                                <View style={{ width: 30 }} />
                                <TouchableOpacity onPress={() => add_lifting1(val1, val2)} style={{ flex: 1, alignItems: 'center', justifyContent: 'center', height: 40, backgroundColor: '#EE1D23', borderRadius: 5 }}>
