...
<div class="container">
    <div class="row justify-content-center">
        <div class="col-md-8">
            <div class="card">
                <div class="card-header">{{ __('Bulk Upload/Update Branch') }}</div>
                {!! Form::open(['route'=> 'branch.upload.save', 'method'=> 'POST' ,'id'=> 'uploadForm', 'files' => true]) !!}
                <div class="card-body">
                    <div id="message">

                    </div>
                    @if(session()->has('branch_import'))
                        <div class="mb-2">
                ...
                <div class="card-footer">
                    ...
                    <a href="{{ asset('format/branch-format.csv') }}" download="branch-format">Download Format</a>
                </div>
                {!! Form::close() !!}
            </div>
...
@push('employee-bulk-upload-blade')
    <script>
      ...
      //  $('#process').css('display', 'block');
            ...
            $.ajax({
                url: "{{ route('branch.upload.progress') }}",
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                },
                type: "POST",
