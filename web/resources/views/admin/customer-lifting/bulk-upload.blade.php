...
<div class="container">
    <div class="row justify-content-center">
        <div class="col-md-8">
            <div class="card">
                <div class="card-header">{{ __('Upload/Update Customer Lifting') }}</div>
                {!! Form::open(['route'=> 'customer-liftings.upload.save', 'method'=> 'POST' ,'id'=> 'uploadForm', 'files' => true]) !!}
                <div class="card-body">
                    <div id="message">

                    </div>
                    @if(session()->has('dealer_import'))
                        <div class="mb-2">
                ...
                <div class="card-footer">
                    ...
                    <a href="{{ asset('format/customer-lifting-format.csv') }}" download="customer-lifting-format">Download Format</a>
                </div>
                {!! Form::close() !!}
            </div>
        </div>
...
@push('dealer-bulk-upload-blade')
    <script>
        ...
        $(document).ready(function(){
          //  getProgress();
            $('#uploadForm').on('submit', function(e){
                ...
                $.ajax({
                    url: "{{ route('customer-liftings.upload.save') }}",
                    headers: {
      ...
      //  $('#process').css('display', 'block');
            ...
            $.ajax({
                url: "{{ route('customer-liftings.upload.progress') }}",
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                },
                type: "POST",
