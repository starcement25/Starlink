...
<div class="container">
    <div class="row justify-content-center">
        <div class="col-md-8">
            <div class="card">
                <div class="card-header">{{ __('Lifting Bulk Updates') }}</div>
                {!! Form::open(['route'=> 'verify.liftings.bulk.update.save', 'method'=> 'POST' ,'id'=> 'uploadForm', 'files' => true]) !!}
                <div class="card-body">
                    <div id="message">

                    </div>
                    @if(session()->has('lifting_bulk_update'))
                        <div class="mb-2">
                ...
                <div class="card-footer">
                    ...
                    <a href="{{ asset('format/lifting_update_format.csv') }}" download="lifting_update_format">Download Format</a>
                </div>
                {!! Form::close() !!}
            </div>
        </div>
...
@push('dealer-bulk-upload-blade')
    <script>
        ...
        $(document).ready(function(){
          //  getProgress();
            $('#uploadForm').on('submit', function(e){
                ...
                $.ajax({
                    url: "{{ route('verify.liftings.bulk.update.save') }}",
                    headers: {
      ...
      //  $('#process').css('display', 'block');
            ...
            $.ajax({
                url: "{{ route('verify.liftings.bulk.update.progress') }}",
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                },
                type: "POST",
