...
<div class="container">
    <div class="row justify-content-center">
        <div class="col-md-8">
            <div class="card">
                <div class="card-header">{{ __('Upload Mason') }}</div>
                {!! Form::open(['route'=> 'mason.upload.save', 'method'=> 'POST' ,'id'=> 'uploadForm', 'files' => true]) !!}
                <div class="card-body">
                    <div id="message">
                    ...
                    <div class="mb-2">
                      ...
                      {{--   <div id="pid" style="display: none">
                            <a href="{{ route('mason.upload.show') }}"><button class="btn btn-success">New Upload</button></a>
                        </div> --}}
                    </div>
                    {{-- @if(session()->has('mason_import'))
...
@push('mason-bulk-upload-blade')
    <script nonce="{{ $cspNonce }}">
      ...
      //  $('#process').css('display', 'block');
            ...
            $.ajax({
                url: "{{ route('mason.upload.progress') }}",
                headers: {
                   ...
                   if(response.success){
                        if(!response.import_status){
                            ...
                            $('#progress').css({'display':'none'}) ;
                              ...
                              window.location.href = "{{ route('mason.upload.show') }}";
