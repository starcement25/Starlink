    ...
    <div class="content px-3">
        ...
        <div class="card">
            <div class="card-body table-responsive">
                <form action="{{route('dealer.accept.liftings')}}" method="GET" id="filterForm">
                    ...
                    <table class="table table-bordered">
                        ...
                        <tbody>
                            @if(count($approvedLists) > 0)
                                @foreach($approvedLists as $approvedList)
                                    <tr>
                                        <td>{{$approvedList['products']}}</td>
                                        <td>{{$approvedList['no_of_bags']}}</td>
                                        <td>{{$approvedList['point']}}</td>
                                        <td>{{$approvedList['lifting_date']}}</td>
                                        <td>{{$approvedList['mason_name']}}</td>
                                        <td>{{$approvedList['mason_phone']}}</td>
                                    </tr>
                                @endforeach
                            @else
                                <tr>
                                    <td colspan="6" class="text-center">No Approved Liftings Founds.</td>
                                </tr>
                            @endif
                        </tbody>
                    </table>
