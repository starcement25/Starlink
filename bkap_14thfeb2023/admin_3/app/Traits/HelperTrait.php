...
trait HelperTrait{
    ...
    {
        $rewardPoint  = Reward::selectRaw("IFNULL(SUM(point), 0) AS `point`")->whereIn('lifting_id', function($q) use($userId){
                            $q->select('id')->from('lifting')->where('user_id', $userId)->pluck('id');
        ...
        $points = ($rewardPoint - $redeemedPoint) ;
